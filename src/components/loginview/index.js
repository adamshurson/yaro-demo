    ...
    render() {
        return (
            <div className="LoginView flex flex-col h-full justify-center items-center bg-grey-lightest">
                ...
                <input placeholder={"Username"} autoFocus={true} value={this.state.username} onChange={(evt) => this.setUsername(evt)} type={"text"} className={"px-4 py-2 bg-transparent text-teal my-2 rounded border-2 border-teal focus:outline-none"} autoComplete="new-password"/>
                <input placeholder={"Password"} value={this.state.password} onChange={(evt) => this.setPassword(evt)} type={"password"} className={"px-4 py-2 bg-transparent text-teal my-2 rounded border-2 border-teal focus:outline-none"} autoComplete="new-password"/>
